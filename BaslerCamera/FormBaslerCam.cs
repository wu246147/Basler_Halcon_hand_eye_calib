...
namespace BaslerCamera
{
    public partial class FormBaslerCam : Form
    {
        ...
        Dictionary<string, HTuple> RobotPoses = new Dictionary<string, HTuple>();

        Dictionary<string, HTuple> CalibPoses = new Dictionary<string, HTuple>();
        ...
        private void btn_OneShot_Click(object sender, EventArgs e)
        {
            ...
            bool bflag = cam.OneShot(out HImage ho_Image);
                ...
                try
                {
                    ...
                    if (robot.GetType() != typeof(KukaRobot))
                    {
                        ...
                        if (robot.Open(textBoxRobotIP.Text, int.Parse(textBoxRobotPort.Text)))
                        {
                            ...
                            {
                                RobotPoses.Add(Image序号.ToString("D3"), hPose);
                            }
                            else
                            {
                    ...
                    listBox图像列表.Items.Add(Image序号.ToString("D3"));
                    listBox图像列表.SelectedIndex = listBox图像列表.Items.Count - 1;

                }
                catch (Exception ex)
                {
                    ShowMessage($"机器人坐标获取失败：" + ex.Message, Color.Red);
                }
            }
            else
            {
        ...
        private void button加载图像_Click(object sender, EventArgs e)
        {
            ...
            openFileDialog.Filter = "PNG(*.png)|*.png|JPEG(*.jpg)|*.jpg|Text(*.txt)|*.txt";
            if (DialogResult.OK == openFileDialog.ShowDialog())
            {
                int selectedIndex = openFileDialog.FilterIndex;
                if (selectedIndex != 2)
                {
                    foreach (var imagePath in openFileDialog.FileNames)
                    {
                        try
                        {
                            string posePath = Path.ChangeExtension(imagePath, ".dat");
                            if (File.Exists(posePath))
                            {
                                ...
                                Images.Add(key, ho_Image);
                                RobotPoses.Add(key, pose);
            ...
            if (listBox图像列表.SelectedIndex != -1)
            {
                ...
                if (DialogResult.OK == sfd.ShowDialog())
                {
                    ...
                    try
                    {
                        ...
                        {
                            case ".png":
                                HOperatorSet.WriteImage(ho_Image, "png", 0, path);//=png 6
                                break;
                            case ".bmp":
                                HOperatorSet.WriteImage(ho_Image, "bmp", 0, path);
                                break;
                            case ".tif":
                                HOperatorSet.WriteImage(ho_Image, "tiff", 0, path);
                                break;
                            case ".jpg":
                                HOperatorSet.WriteImage(ho_Image, "jpeg", 0, path);//=jpeg 50~60
                                break;
                            default:
                                return;
                        }
                        ...
                        if (RobotPoses.ContainsKey(key))
                        {
                            HOperatorSet.WritePose(RobotPoses[key], Path.ChangeExtension(path, ".dat"));
                        }  
                        ShowMessage("图片保存成功");
                    }
                    catch (Exception ex)
            ...
            if (Images.Count > 0)
            {
                ...
                if (DialogResult.OK == folder.ShowDialog())
                {
                    foreach (var name in Images.Keys)
                    {
                        try
                        {
                            string filePath = $"{folder.SelectedPath}\\{Path.GetFileNameWithoutExtension(name)}.png";
                            ...
                            {
                                i++;
                                filePath = $"{folder.SelectedPath}\\{Path.GetFileNameWithoutExtension(name)}({i}).png";
                            }

                            HOperatorSet.WriteImage(Images[name], "png", 0, filePath);//=png 6

                            if (RobotPoses.ContainsKey(name))
                            {
                                HOperatorSet.WritePose(RobotPoses[name], Path.ChangeExtension(filePath, ".dat"));
                            }
                        }
                        catch (Exception ex)
                        {
        ...
        private void button手眼标定_Click(object sender, EventArgs e)
        {
            if (Images.Count > 0)
            {
                ...
                try
                {
                    ...
                    {
                        if(!RobotPoses.ContainsKey(item))
                        {
                            ShowMessage($"图片\"{item}\"没有保存对应的机器人姿态", Color.Red);
                            continue;
                        }
                        ...
                        try
                        {
                            HOperatorSet.FindCalibObject(Images[item], hv_CalibHandle, 0, 0, id, hv_TmpCtrl_FindCalObjParNames, hv_TmpCtrl_FindCalObjParValues);
                            HOperatorSet.SetCalibData(hv_CalibHandle, "tool", id, "tool_in_base_pose", RobotPoses[item]);
                            id++;
                        }
                        catch (Exception ex)
                        {
        ...
        private void button保存外参_Click(object sender, EventArgs e)
        {
            if (ToolInCamPose != null)
            {
                ...
                if (DialogResult.OK == sfd.ShowDialog())
                {
                    ...
                    {
                        string path = sfd.FileName;
                        HOperatorSet.WritePose(ToolInCamPose, path);
                        ShowMessage("保存外参成功");
                    }
                    catch (Exception ex)
                    {
        ...
        private void button_check_hand_calib_Click(object sender, EventArgs e)
        {
            if (Images.Count > 0 && ToolInCamPose != null&& listBox图像列表.SelectedIndex != -1 && textBox描述文件路径.Text!="")
            {
                if (hv_CalibHandle != null)
                {
                    ...
                    //Dictionary<string, HTuple> PosesDist = new Dictionary<string, HTuple>();

                    //int id = 0;
                    //foreach (string item in Images.Keys)
                    //{
                    //    HOperatorSet.GetCalibData(hv_CalibHandle, "calib_obj_pose", (new HTuple(0)).TupleConcat(id), "pose", out HTuple hv_CamPoseCal);
                    //    id++;
                    //    //转机器人坐标
                    //    HOperatorSet.PoseToHomMat3d(RobotPoses[item], out HTuple hv_robotPoseMat);
                    //    HOperatorSet.AffineTransPoint3d(hv_robotPoseMat, hv_CamPoseCal[0], hv_CamPoseCal[1], hv_CamPoseCal[2], out HTuple robotX, out HTuple robotY, out HTuple robotZ);

                    //    HTuple hv_dist = new HTuple();
                    //    hv_dist[0] = robotX - hv_CalObjInBasePose[0];
                    //    hv_dist[1] = robotY - hv_CalObjInBasePose[1];
                    //    hv_dist[2] = robotZ - hv_CalObjInBasePose[2];

                    //    ShowMessage($"标定板计算在基座坐标系下的坐标：({robotX.D},{robotY.D},{robotZ.D})") ;

                    //    ShowMessage($"标定板误差计算：" + hv_dist.ToString());

                    //    PosesDist.Add(item, hv_dist);
                    //}


                    //ShowMessage($"标定板总误差计算：" + hv_dist.ToString());
